...
> Lives in `arena/`. The repository root is now **PadhAI** (JEE Main study tracker) —
> see the root `README.md`. This app is served at `/arena/`.
...
```bash
open arena/index.html           # or double-click
# or serve (recommended):
python3 -m http.server 8080     # http://localhost:8080/arena/
```
